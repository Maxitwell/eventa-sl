...
export default function CreateEvent() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!title || !date || !time || !location || !description || !organizerName) {
        ...
        if (isFreeEvent) { computedCapacity = 1000; minPrice = 0; } // Fallbacks for free events
        if (minPrice === -1) minPrice = 0; // if all private or free

        try {
            const { createEvent } = await import("@/lib/db");

            let imageUrl = "https://images.unsplash.com/photo-1540039155732-d69288f5e6a9?q=80&w=2669&auto=format&fit=crop"; // Default
    ...
    const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // clear input so same image can be selected
        e.target.value = "";
    };
