                ...
                if (digitsOnly.length >= 6) {
                    ...
                    try {
                        const pawapayPayload = {
                            depositId, amount: String(currentPrice), currency: 'SLE', country: 'SLE', correspondent: 'ORANGE_SLE', payer: { type: 'MSISDN', address: { value: phone } }, statementDescription: `Eventa ${String(session.eventName ?? '').replace(/[^a-zA-Z0-9 ]/g, '')}`.slice(0, 22), customerTimestamp: new Date().toISOString(), metadata: [{ fieldName: 'channel', fieldValue: 'whatsapp', isPII: false }, { fieldName: 'waFrom', fieldValue: from, isPII: true }],
                        };
